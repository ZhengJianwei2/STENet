...
from models.Decom_Backbone import ResNet3D
from models.resnet import resnet18


class AFCF3D_Net(nn.Module):
    def __init__(self, channel=32):
        super(AFCF3D_Net, self).__init__()
        self.res = resnet18()
        self.res.load_state_dict(torch.load('./pretrained/resnet18-5c106cde.pth'))
        self.resnet = ResNet3D(self.res)
...
if __name__ == '__main__':
    ...
    input1 = torch.randn(1, 3, 256, 256).cuda(5)
    input2 = torch.randn(1, 3, 256, 256).cuda(5)

    model = AFCF3D_Net().cuda(5)
