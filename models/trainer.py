...
class CDTrainer():
    ...
    def __init__(self, args, dataloaders):
        ...
        self.optimizer_G = optim.SGD(self.net_G.parameters(), lr=self.lr,
                                     momentum=0.9,
                                     weight_decay=5e-4)
        # self.optimizer_G = optim.AdamW(self.net_G.parameters(), lr=self.lr, eps=1e-8, weight_decay=0.01)
        ...
        if args.loss == 'ce':
            ...
            self._pxl_loss = losses.BCEDiceLoss
        else:
    ...
    def _backward_G(self):
        ...
        # self.G_loss = losses.BCEDiceLoss(self.G_pred1, gt)

        self.G_loss = losses.BCEDiceLoss(self.G_pred1, gt) + losses.BCEDiceLoss(
            self.G_pred2, gt) + 0.5*losses.BCEDiceLoss(self.G_pred_middle1, gt) + 0.5*losses.BCEDiceLoss(
            self.G_pred_middle2, gt) + 0.5*losses.BCEDiceLoss(self.G_pred_middle3, gt)

        # self.G_loss = losses.BCEDiceLoss(self.G_pred1, gt) + losses.BCEDiceLoss(
        #     self.G_pred2, gt) + losses.BCEDiceLoss(self.G_pred_middle1, gt)
